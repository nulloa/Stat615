...
\begin{enumerate}
  ...
  mcmc_horseshoe <- function(n_reps, y, group, initial_values){
    ...
    lambda <- as.numeric(unlist(initial_values["lambda"]))
    sigma2 <- as.numeric(initial_values["sigma2"])
    tau2   <- as.numeric(initial_values["tau2"])
    theta  <- as.numeric(unlist(initial_values["theta"]))
    ...
    keep_lambda <- matrix(rep(NA, n_reps*G), ncol=G, nrow=n_reps)
    ...
    n <- length(y)
    n_g <- tmpdf %>% group_by(group) %>% summarise(n_g = length(y))
    n_g <- as.numeric(n_g[,2])
    ybar_g <- tmpdf %>% group_by(group) %>% summarise(ybar_g = mean(y))
    ...
    for(i in 1:n_reps){
      for(g in 1:G){
        ...
        sigmaTheta <- ((1/lambda[g]^2*tau2^2) + (n_g[g]/sigma2^2))^(-1)
        muTheta    <- sigmaTheta*(ybar_g[g]*n_g[g]/sigma2^2)
        ...
        # Update Storage
        keep_lambda[i,g]  <- lambda
        keep_theta[i,g]   <- theta
      }
      
      tmpd <- data.frame(y, group, theta = keep_theta[i,g][group])
      SSE <- sum(tmpd %>%group_by(Group) %>%summarise(tmp = (y-theta)^2))
      a <- n/2
      b <- 0.5*SSE
