...
\begin{enumerate}
  ...
  %%%%% Third Part %%%%%
  \item
  Here we set up the data simulation:
  <<data_setup, echo=true, messages=FALSE>>=
  library("dplyr")
  library("Rcpp")
...
set.seed(1)
G = 25
theta = c(rep(0,G-5), rnorm(5,c(35,10,15,15,20), sd=5))
trueTheta = theta
d = data.frame(group = rep(1:G, each=10))
...
d$y = rnorm(nrow(d), d$theta)
  ...
  In this plot we can see the CI of the groups plotted by each model.
 <<plot_theta, messgae=FALSE, echo=FALSE>>=
  library(reshape2)
  library(ggplot2)
...
facet_grid(~Model) +
geom_segment() +
geom_segment(aes(x = TrueTheta, xend = TrueTheta), color="red", size=10) +
