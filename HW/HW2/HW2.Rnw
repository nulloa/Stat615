...
\begin{enumerate}
 ...
 estl <-    mlambda %>%
  ...
  mcmc_horseshoe2 <- function(n_reps, y, group, initial_values){
    require("MCMCpack")
    require("dplyr")
  ...
  n_reps = 1000
  HorseshoeRes2 <- mcmc_horseshoe2(n_reps, y = d$y, group = d$group, initial_values = initial_values)
 ...
 estl <-    mlambda %>%
  ...
  ggplot() + geom_histogram(aes(x = HorseshoeRes$tau2))   + geom_vline(xintercept = as.numeric(initial_values["tau2"]), color="red")
  ggplot() + geom_line(aes(x = 1:n_reps ,y = HorseshoeRes2$tau2))
  
  summary(HorseshoeRes$tau2)
  summary(HorseshoeRes2$tau2)

  
  # Acceptance probability
  length(unique(HorseshoeRes2$tau2))/length(HorseshoeRes2$tau2)
  @
